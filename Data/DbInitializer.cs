...
public static class DbInitializer
{
    public static void Seed(EventifyDbContext context)
    {
        ...
        if (!context.TicketOptions.Any(t => t.EventItemId == techSummitId))
        {
        ...
        if (!context.Speakers.Any(s => s.EventItemId == techSummitId))
        {
        ...
        foreach (var review in reviewSeeds)
        {
            var exists = context.Reviews.Any(r =>
                r.EventItemId == review.EventItemId &&
        ...
        var techSummit = context.Events.First(e => e.Id == techSummitId);
        var techSummitReviews = context.Reviews.Where(r => r.EventItemId == techSummitId).ToList();
        if (techSummitReviews.Count > 0)
        {
            techSummit.ReviewCount = techSummitReviews.Count;
            techSummit.Rating = Math.Round(techSummitReviews.Average(r => r.Rating), 1);
        }
